...
public class RobotContainer {
  ...
  private static BooleanSupplier m_firstBeamBreakSees = () -> false;
  private static BooleanSupplier m_firstBeamBreakDontSees = () -> false;
  ...
  public RobotContainer() {
    ...
    m_canShoot = new Trigger(() ->(m_shooter.isAtSetpoint() && m_arm.isAtSetpoint() && VisionSubsystem.getTv() && PoseEstimatorSubsystem.isCentered())); //TODO add swerve slow spped
    m_firstBeamBreakSees = () -> m_intake.firstBeamBreak();
    m_firstBeamBreakDontSees = () -> !m_intake.firstBeamBreak();
  ...
  public void intakeBinding(){
    ...
    drivercontroller.L2().and(m_firstBeamBreakDontSees).onTrue(new IntakeEatUntilHasNote(m_intake, 0.7, true).andThen(new IntakeGlubGlub(m_intake, true)).andThen(new IntakeEatUntilHasNote(m_intake, 0.5, false)).andThen(new IntakeGlubGlub(m_intake, false)));
    drivercontroller.R2().whileTrue(new IntakeForTime(m_intake, -0.3, 2.0));
    drivercontroller.R1().debounce(0.4).and(m_canShoot).onTrue(new IntakeForTime(m_intake, 0.3, 1.0));
  }
