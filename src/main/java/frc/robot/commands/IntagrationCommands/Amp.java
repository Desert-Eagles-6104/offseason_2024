...
public class Amp extends SequentialCommandGroup {
  ...
  public Amp(IntakeSubsystem intake, ArmSubsystem arm, ShooterSubsystem shooter) {
    addCommands(new InstantCommand(() -> arm.ControlSoftLimit(false)),new SmartPreset(shooter, arm, 75, 700, false),new WaitCommand(0), new IntakeForTime(intake, 0.6, 0.3).alongWith(new SmartPreset(shooter, arm, 100, 700, false)), new WaitCommand(0.5), new SmartPreset(shooter, arm, 20, 0), new WaitCommand(0.1) ,new InstantCommand(() -> arm.ControlSoftLimit(true)));
  }
}
